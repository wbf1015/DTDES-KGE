...
AttH需要double的关系嵌入维度
"""
class AttH_Reverse(nn.Module):
    def __init__(self, args):
        super(AttH_Reverse, self).__init__()
        self.args = args
        ...
        self.rel_diag = nn.Embedding(args.nrelation, 2* args.target_dim)
        self.rel_diag.weight.data = 2 * torch.rand((args.nrelation, 2*args.target_dim), dtype=self.data_type) - 1.0

        self.context_vec = nn.Embedding(args.nrelation, args.target_dim)
        self.context_vec.weight.data = self.init_size * torch.randn((args.nrelation, args.target_dim), dtype=self.data_type)
        ...
        if self.args.data_type=='double':
            self.scale = torch.Tensor([1. / np.sqrt(self.target_dim)]).double().cuda()
        else:
            self.scale = torch.Tensor([1. / np.sqrt(self.target_dim)]).float().cuda()
    ...
    def get_queries(self, head, relation, tail, sample):
        ...
        head = head
        rot_mat, ref_mat = torch.chunk(self.rel_diag(positive_sample[:, 1]), 2, dim=1)
        rot_q = givens_rotations(rot_mat, head).view((-1, 1, self.target_dim))
        ref_q = givens_reflection(ref_mat, head).view((-1, 1, self.target_dim))
        cands = torch.cat([ref_q, rot_q], dim=1)
        context_vec = self.context_vec(positive_sample[:, 1]).view((-1, 1, self.target_dim))
        att_weights = torch.sum(context_vec * cands * self.scale, dim=-1, keepdim=True)
        att_weights = self.act(att_weights)
        att_q = torch.sum(att_weights * cands, dim=1)
        lhs = expmap0(att_q, c)
        rel, _ = torch.chunk(relation, 2, dim=1)
        rel = expmap0(rel, c)
