...
"""
class RotH_Reverse(nn.Module):
    def __init__(self, args):
        super(RotH_Reverse, self).__init__()
        self.args = args
    ...
    def get_queries(self, head, relation, tail, sample):
        ...
        head = expmap0(self.entity(positive_sample[:, 0]), c)
        rel1, rel2 = torch.chunk(self.rel(positive_sample[:, 1]), 2, dim=1)
        rel1 = expmap0(rel1, c)
        rel2 = expmap0(rel2, c)
        lhs = project(mobius_add(head, rel1, c), c)
        res1 = givens_rotations(self.rel_diag(positive_sample[:, 1]), lhs)
        res2 = mobius_add(res1, rel2, c)
        return (res2, c), self.bh(positive_sample[:, 0])
